    ...
    export class Treeview {
        ...
        _createControls() {
            if (this.options.searchEnabled) {
                ...
                this.treeSearchInput.setAttribute('class', 'treeview-search-input form-control form-control-sm');
        ...
        _expandAll() {
            ...
            allExpandableNodes.forEach(li => {
                if (!li.classList.contains('expanded')) {
                    ...
                    if (expander) expander.innerHTML = '<i class="bi bi-dash-square"></i>';
        ...
        _collapseAll() {
            ...
            for (let i = allExpandableNodes.length - 1; i >= 0; i--) {
                ...
                if (li.classList.contains('expanded')) {
                    ...
                    const expander = li.querySelector('.treeview-expander');
                    if (expander) expander.innerHTML = '<i class="bi bi-plus-square"></i>';
                    const childUl = li.querySelector('ul');
                    if (childUl) {
        ...
        _renderTree(data, parentElement) {
            ...
            data.forEach(node => {
                ...
                if (node.children && node.children.length > 0) {
                    ...
                    expanderOrPlaceholder.innerHTML = this.options.initiallyExpanded ? '<i class="bi bi-dash-square"></i>' : '<i class="bi bi-plus-square"></i>';
                } else {
                    expanderOrPlaceholder = document.createElement('span');
                ...
                if (node.children && node.children.length > 0) {
                    ...
                    expanderOrPlaceholder.addEventListener('click', (event) => {
                        ...
                        if (childUl) {
                            if (li.classList.contains('expanded')) {
                                li.classList.remove('expanded');
                                expanderOrPlaceholder.innerHTML = '<i class="bi bi-plus-square"></i>';
                                childUl.style.height = `${childUl.scrollHeight}px`;
                                requestAnimationFrame(() => {
                                    childUl.style.height = '0px';
                                });
                                ...
                                li.classList.add('expanded');
                                expanderOrPlaceholder.innerHTML = '<i class="bi bi-dash-square"></i>';
                                childUl.style.height = '0px';
                                requestAnimationFrame(() => {
                                    childUl.style.height = `${childUl.scrollHeight}px`;
                                });
